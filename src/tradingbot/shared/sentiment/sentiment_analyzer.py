...
async def call_local_model(
    session: Optional[aiohttp.ClientSession], text: str, language: str
) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            async with local_session.get(health_check_url) as health_check:
                if health_check.status != 200:
                    ...
                    raise ConnectionError(
                        f"Ollama service health check failed with status {health_check.status}"
                    )
            ...
            raise ConnectionError(
                f"Failed to connect to Ollama service at {endpoint}: {str(e)}"
            )
    ...
    try:
        ...
        async with local_session.post(generate_url, json=data) as resp:
            if resp.status != 200:
                ...
                raise Exception(
                    f"Ollama API returned status {resp.status}: {error_text}"
                )
            ...
            if not isinstance(sentiment_data, dict):
                raise ValueError(
                    f"Invalid response format, expected dict but got: {type(sentiment_data)}"
                )
...
async def call_remote_model(
    session: aiohttp.ClientSession, text: str, language: str
) -> Dict[str, Any]:
    if not text:
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a sentiment analysis expert. Provide sentiment analysis results in a consistent format: one word (POSITIVE/NEGATIVE/NEUTRAL) followed by a confidence score (0-1).",
            },
...
def adjust_meme_sentiment(
    result: Dict[str, Any], is_meme: bool = False
) -> Dict[str, Any]:
    if not is_meme:
        return result
...
async def analyze_text(
    text: Union[str, bytes], language: str = "en", is_meme: bool = False
) -> Dict[str, Any]:
    ...
    try:
        if AI_MODEL_MODE != "REMOTE":
            try:
                ...
                if (
                    AI_MODEL_MODE == "LOCAL"
                    and not os.getenv("ALLOW_REMOTE_FALLBACK", "true").lower() == "true"
                ):
                    raise e

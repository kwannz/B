...
class ValuationAgent(BaseAgent):
    ...
    async def calculate_valuation(self, symbol: str) -> Dict[str, Any]:
        ...
        if "nvt_ratio" in self.valuation_methods:
            if volume > 0:
                nvt_ratio = price / (volume / circulating_supply if "market_cap" in valuations else volume)
        ...
        if sentiment_data:
            sentiment_score = sentiment_data.get("sentiment", {}).get("combined", {}).get("score", 0.5)
        ...
        await self.db_manager.mongodb.token_valuations.insert_one({
            **valuation_result,
            "meta_info": {
                ...
                "sentiment_data_id": str(sentiment_data["_id"]) if sentiment_data else None,

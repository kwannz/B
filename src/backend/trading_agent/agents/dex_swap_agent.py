...
class DexSwapAgent(BaseTradingAgent):
    ...
    def calculate_indicators(self, prices: pd.Series) -> Dict[str, Any]:
        ...
        return {
            ...
            ) if not (ma_fast.empty or ma_slow.empty) else False
        }
    ...
    async def get_trade_signal(self, token: str, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        try:
            price = Decimal(str(market_data.get("price", "0")))
            volume = Decimal(str(market_data.get("volume", "0")))
            timestamp = datetime.fromisoformat(market_data.get("timestamp", datetime.utcnow().isoformat()))
            ...
            rsi = Decimal(str(indicators["rsi"])) if indicators["rsi"] is not None else None
            if rsi is None:
                return None
    ...
    async def execute_strategy(self, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        try:

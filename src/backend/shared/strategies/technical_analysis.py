...
class TechnicalAnalysisStrategy:
    ...
    def __init__(self, config):
        ...
        self.rsi_period = self._validate_positive_int(params.get("rsi_period", 14), "rsi_period")
        self.rsi_overbought = self._validate_rsi_level(params.get("rsi_overbought", 70), "rsi_overbought")
        self.rsi_oversold = self._validate_rsi_level(params.get("rsi_oversold", 30), "rsi_oversold")
        
        # Validate MA parameters
        self.ma_short_period = self._validate_positive_int(params.get("ma_short_period", 10), "ma_short_period")
        self.ma_long_period = self._validate_positive_int(params.get("ma_long_period", 20), "ma_long_period")
        ...
        self.min_volume = self._validate_positive_float(params.get("min_volume", 1000), "min_volume")
    ...
    def _check_divergence(self, prices: List[float], rsi_values: List[float]) -> Optional[str]:
        ...
        # 顶背离检测（价格新高，RSI未新高）
        if (price_points[-1] > price_points[-3] and 
            rsi_points[-1] < rsi_points[-3]):
            return "bearish"
    ...
    async def calculate_signals(self, market_data: Optional[List[Dict]]) -> Dict[str, Any]:
        ...
        try:
            if market_data is None or not market_data:
                return {
                    "signal": "neutral",
                    "confidence": 0.0,
                    "reason": "error: invalid data" if market_data is None else "error: no data"
                }
    ...
    async def update_positions(self, tenant_id: str, market_data: Optional[Dict]) -> None:

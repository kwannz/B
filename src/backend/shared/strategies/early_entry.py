...
class EarlyEntryStrategy:
    ...
    def _calculate_rsi(self, prices: List[float]) -> Optional[float]:
        ...
        if len(prices) >= 2 and all(current < next_ for current, next_ in zip(prices, prices[1:])):
            return 100.0
    ...
    async def calculate_signals(self, market_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            required_fields = ["volume", "price", "liquidity", "market_cap", "listing_time"]
            if any(f not in latest for f in required_fields):
                return {"signal": "neutral", "confidence": 0.0, "reason": "missing_fields"}
            ...
            age_hours = (datetime.utcnow() - datetime.fromisoformat(latest["listing_time"].replace('Z', '+00:00'))).total_seconds() / 3600
            ...
            market_confidence = (liquidity_score * 0.4 + volume_score * 0.4 + age_score * 0.2)
            tech_confidence = 0.7 if (rsi and rsi <= self.rsi_oversold) or volume_surge else 0.0
            final_confidence = (market_confidence * 0.6 + tech_confidence * 0.4)
            
            # Signal generation
            signal = "buy" if (
                liquidity_score >= 0.8 and 
                volume_score >= 0.8 and
    ...
    def _check_divergence(self, prices: List[float], rsi_values: List[float]) -> Optional[str]:
        """檢查價格與RSI的背離現象"""
        if len(prices) < self.divergence_window or len(rsi_values) < self.divergence_window:
            return None
            
        price_trend = np.polyfit(range(self.divergence_window), prices[-self.divergence_window:], 1)[0]
        rsi_trend = np.polyfit(range(self.divergence_window), rsi_values[-self.divergence_window:], 1)[0]
        
        if price_trend < -self.divergence_threshold and rsi_trend > self.divergence_threshold:
            return "bullish"
        if price_trend > self.divergence_threshold and rsi_trend < -self.divergence_threshold:
            return "bearish"
        return None

...
def upgrade() -> None:
    ...
    op.create_table('tenants',
        ...
        sa.Column('status', ENUM('ACTIVE', 'SUSPENDED', 'DELETED', name='tenantstatus', create_type=False), nullable=False),
    ...
    op.create_table('news_articles',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('tenant_id', sa.Integer(), nullable=False),
        sa.Column('source', ENUM('COINDESK', 'COINTELEGRAPH', 'DECRYPT', name='newssource', create_type=False), nullable=False),
    ...
    op.create_index('idx_article_published', 'news_articles', [sa.text('published_at DESC')])
    ...
    op.create_table('trades',
        ...
        sa.Column('remaining_amount', sa.Numeric(precision=36, scale=18), nullable=True),
        ...
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
        sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['parent_trade_id'], ['trades.id'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('id')
    )

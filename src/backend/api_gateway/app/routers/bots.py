...
class Bot(BaseModel):
    ...
    exists: bool = True
    uptime: str = Field(default_factory=lambda: f"Active since {datetime.utcnow().isoformat()}")
        ...
        for bot_id, bot_data in bots_db.items():
            try:
                bot = {
                    ...
                    "last_updated": bot_data.get("last_updated", datetime.utcnow().isoformat())
                }
                bots.append(bot)
            except Exception as e:
...
async def get_bot(bot_id: str):
    try:
        ...
        if not bot_data:
            logger.warning(f"Bot {bot_id} not found. Available bots: {list(bots_db.keys())}")

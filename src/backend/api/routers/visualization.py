...
async def get_performance_data(
    ...
    try:
        ...
        profit_factor = abs(df[df["pnl"] > 0]["pnl"].sum()) / abs(df[df["pnl"] < 0]["pnl"].sum()) if len(df[df["pnl"] < 0]) > 0 else float("inf")
        
        return {
            "pnl_chart": df["cumulative_pnl"].resample("1H").last().fillna(method="ffill").to_dict(),
...
async def get_position_distribution(
    ...
    try:
        ...
        largest_position = max(positions, key=lambda p: abs(p["quantity"] * p["current_price"]))
        ...
        return {
            ...
            "largest_position": {
                ...
                "value": round(abs(largest_position["quantity"] * largest_position["current_price"]), 2),
                "percentage": round(position_values[largest_position["symbol"]] / total_value * 100, 2)
            }
        }
...
async def get_strategy_performance(
    ...
    try:
        ...
        return {
            "performance_chart": df["cumulative_pnl"].resample("1D").last().fillna(method="ffill").to_dict(),
            "metrics": {

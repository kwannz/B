...
class JupiterClient:
    ...
    async def get_quote(self, input_mint: str, output_mint: str, 
                       amount: int, slippage_bps: Optional[int] = None) -> Dict[str, Any]:
        await self.start()
    ...
    async def get_swap_instruction(self, quote_response: Dict[str, Any]) -> Dict[str, Any]:
        await self.start()
        assert self.session is not None
            
        try:
            async with self.session.post(f"{self.base_url}/swap", json=quote_response) as response:
                if response.status != 200:
                    error_text = await response.text()
                    logger.error(f"Jupiter swap instruction error: {error_text}")
                    return {"error": f"Jupiter swap instruction error: {response.status}"}
    ...
    async def get_price(self, input_mint: str, output_mint: str, amount: int) -> Optional[Decimal]:
    ...
    async def get_routes(self, input_mint: str, output_mint: str, 
                        amount: int, slippage_bps: Optional[int] = None) -> Dict[str, Any]:
        await self.start()
        assert self.session is not None
        ...
        try:
            async with self.session.get(f"{self.base_url}/routes", params=params) as response:
                if response.status != 200:
                    error_text = await response.text()

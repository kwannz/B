...
class MarketMaker:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.meme_spread_multiplier = Decimal(str(config.get("meme_spread_multiplier", "1.5")))
    ...
    def calculate_spread(self, volume: Decimal, is_meme: bool = False) -> Decimal:
        base_spread = self.min_spread + (
            ...
            (Decimal("1") - (min(volume, self.volume_threshold) / self.volume_threshold))
        )
    ...
    async def get_market_depth(self, token: str, quote_token: str) -> Dict[str, Any]:
        try:
            depth = await self.dex_client.get_liquidity(
                "jupiter",
                token,
                quote_token
            )
    ...
    async def calculate_order_prices(self, market_price: Decimal, spread: Decimal) -> Dict[str, Decimal]:
    ...
    async def should_update_orders(self, token: str, current_prices: Dict[str, Decimal]) -> bool:
        ...
        for order in orders:
            price_diff = abs(Decimal(str(order["price"])) - current_prices[order["side"].lower()])
            if price_diff / Decimal(str(order["price"])) > self.min_spread:
                return True
                
        return False
        
    async def update_orders(self, token: str, quote_token: str, 
                          market_data: Dict[str, Any], is_meme: bool = False) -> List[Dict[str, Any]]:
        try:

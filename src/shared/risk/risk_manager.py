...
class RiskConfig:
    ...
    def validate(self):
        ...
        if not all(0 < x <= 2 for x in self.DYNAMIC_TAKE_PROFIT_LEVELS):
            raise ValueError("DYNAMIC_TAKE_PROFIT_LEVELS values must be between 0 and 2 (200%)")
...
class RiskAssessment(BaseModel):
    ...
    is_stale: bool = False
    rate_limit_info: Dict[str, Any] = {"is_limited": False, "remaining": 100, "reset": 0.0}
...
class RiskManager:
    ...
    async def _check_rate_limit(self, key: str, max_requests: int = 10, window: float = 60.0) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_limited:
                ...
                self._add_recommendation(f"Rate limit exceeded for {key} - wait {reset_time - current_time:.1f}s")
                return {
                    "limit": max_requests,
                    "remaining": 0,
                    "reset": reset_time,
    ...
    async def assess_trade(self, trade_params: Dict[str, Any]) -> RiskAssessment:
        ...
        try:
            ...
            if symbol:
                ...
                if cached_data:
                    try:
                        ...
                        if current_time - cached["timestamp"] < 300:  # 5 minute staleness check
                ...
                if not self._metrics:
                    ...
                    if any(self._metrics[f] <= 0 for f in required_fields):
                        return RiskAssessment(
                            ...
                            risk_reward_ratio=0.0,
                            market_conditions_alignment=0.0,
                            recommendations=["Invalid market data parameters", "Check price, volume, and liquidity values"],
                    ...
                    try:
                        await set_cache(cache_key, json.dumps(self._metrics), expire=300)
                    except Exception as e:
            ...
            async with self._risk_assessment_timeout():
                basic_validation = await self._validate_basic_params(trade_params)
                if not basic_validation.is_valid:
                    logger.warning("Trade validation failed", extra={"params": trade_params})
                    return basic_validation
                ...
                if position_size <= 0:
                    return RiskAssessment(
                        ...
                        risk_reward_ratio=0.0,
                        market_conditions_alignment=0.0,
                        recommendations=["Invalid position size", "Check account balance and leverage settings"],
                        ...
                        expected_slippage=float(trade_params.get("spread", 0.001)) * 1.5,
                        margin_requirements={"required": 0.0, "available": 0.0},
                        correlation_factor=0.0,
                        liquidity_score=float(trade_params.get("liquidity", 0.0)),
                        volume_profile={"volume": float(trade_params.get("volume", 0.0))},
            ...
            if not market_check["is_valid"]:
                return RiskAssessment(
                    ...
                    recommendations=market_check.get("recommendations", ["Poor market conditions"]),
                    ...
                    market_data_source=market_check.get("market_data", {}).get("source", "live"),
                    is_stale=market_check.get("is_stale", False)
                )
            
            metrics_dict = await self._calculate_risk_metrics(trade_params, position_size)
            initial_assessment = RiskAssessment(
                ...
                dynamic_position_size=metrics_dict.get("adjusted_position", position_size),
                ...
                margin_requirements=metrics_dict.get("margin_requirements", {"required": 0.0, "available": 0.0}),
        ...
        try:
            ...
            try:
                ...
                return RiskAssessment(
                    ...
                    risk_reward_ratio=0.0,
                    market_conditions_alignment=0.0,
                    recommendations=["Invalid trade parameters", "Amount and price must be numeric"],
            ...
            if amount <= 0 or price <= 0:
                return RiskAssessment(
                    ...
                    risk_reward_ratio=0.0,
                    market_conditions_alignment=0.2,
                    recommendations=["Invalid trade parameters", "Amount and price must be positive"],
            ...
            return RiskAssessment(
                ...
                slippage=spread,
                expected_slippage=spread * 1.5,
                margin_requirements={"required": amount * price * 0.1, "available": float('inf')},
                correlation_factor=0.0,
                liquidity_score=liquidity,
                volume_profile={"volume": volume, "liquidity": liquidity},
                market_data_source="live",
            ...
            if params["leverage"] > self.config.MAX_LEVERAGE * (2.0 if params["is_meme_coin"] else 1.5):
            ...
            params["liquidity"] = max(self.config.MIN_LIQUIDITY * 0.1, params["liquidity"])
    ...
    async def _calculate_position_size(self, params: Dict[str, Any]) -> float:
        try:
            ...
            margin_used = sum(float(pos.get("margin_used", 0)) for pos in existing_positions)
            margin_ratio = margin_used / account_size if account_size > 0 else float('inf')
            if margin_ratio > 0.7:
                base_size *= 0.6
            
            # Drawdown check
            unrealized_pnl = sum(float(pos.get("unrealized_pnl", 0)) for pos in existing_positions)
            ...
            if volatility > self.config.VOLATILITY_SCALE_THRESHOLD:
                vol_reduction = max(0.3, 1.0 - (volatility - self.config.VOLATILITY_SCALE_THRESHOLD) / 2.0)
    ...
    async def _check_market_conditions(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_impact = (position_value / market_data["liquidity"]) if market_data["liquidity"] > 0 else float('inf')
        volume_scale = min(0.5, market_data["volume"] / (position_value * 1000)) if position_value > 0 else 0.1
        ...
        base_slippage = market_impact + (market_data["spread"] * 0.25)  # Higher spread impact
        volume_factor = max(0.6, min(1.1, market_data["volume"] / (position_value * 100))) if position_value > 0 else 0.6
        ...
        if expected_slippage > self.config.MAX_SLIPPAGE * 0.6:  # Stricter slippage threshold
            return {
                "is_valid": False,
        ...
        if market_data["volatility"] > max_volatility:
            if market_data["volatility"] > max_volatility * 1.5:  # Reject extremely volatile conditions
                return {
                    "is_valid": False,
            ...
            vol_scale = max(0.3, 1.0 - (market_data["volatility"] - max_volatility) / max_volatility)
        ...
        min_liquidity = self.config.MIN_LIQUIDITY * (0.4 if is_meme else 0.8)  # Higher liquidity requirements
        if market_data["liquidity"] < min_liquidity:
            if market_data["liquidity"] < min_liquidity * 0.5:  # Reject extremely low liquidity
                return {
                    "is_valid": False,
        ...
        volatility_factor = max(0.7, market_data["volatility"] / 2.0)  # Stricter volatility scaling
        ...
        if market_data["volume"] < min_volume:
            ...
            vol_scale = max(0.3, market_data["volume"] / min_volume)  # Stricter volume scaling
            position_scale *= vol_scale
            market_conditions.append({
                "condition": "volume",
                "status": "warning",
        ...
        if market_data["spread"] > max_spread:
            spread_scale = max(0.4, 1.0 - (market_data["spread"] - max_spread) / max_spread)
        ...
        return {
            ...
            "market_conditions_alignment": market_conditions_alignment,
            "recommendations": [c["message"] for c in market_conditions] if market_conditions else ["Market conditions acceptable"],
            "scale_factors": {c["condition"]: c.get("scale", 1.0) for c in market_conditions}
        }
    ...
    async def _calculate_risk_metrics(self, params: Dict[str, Any], position_size: float) -> Dict[str, Any]:
        ...
        try:
            ...
            volatility = float(params.get("volatility", 1.0))
            liquidity = float(params.get("liquidity", self.config.MIN_LIQUIDITY))
            volume = float(params.get("volume", liquidity * 0.05))  # Reduced default volume requirement
            spread = float(params.get("spread", 0.001))
            account_size = float(params.get("account_size", 100000.0))
            leverage = float(params.get("leverage", 1.0))
            stop_loss = float(params.get("stop_loss", price * (0.98 if is_buy else 1.02)))
            take_profit = float(params.get("take_profit", price * (1.03 if is_buy else 0.97)))
            
            # More lenient volume validation
            min_volume = max(1000, position_size * price * 0.5)  # Only require 0.5x position value in volume
            ...
            base_impact = (position_value / liquidity) if liquidity > 0 else float('inf')
            volume_scale = min(0.5, volume / (position_value * 1000)) if position_value > 0 else 0.1
            volatility_scale = min(1.02, 1.0 + volatility * 0.01)
            market_impact = round(base_impact * volume_scale * volatility_scale * 0.1, 6)
            
            # Slippage calculation
            base_slippage = market_impact + (spread * 0.1)
            volume_factor = max(0.95, min(1.05, volume / (position_value * 100))) if position_value > 0 else 1.0
            ...
            volatility_factor = min(1.5, max(1.0, volatility / self.config.VOLATILITY_SCALE_THRESHOLD))
            ...
            trailing_stop = price * (1 + (take_profit_pcts[0] * self.config.TRAILING_STOP_ACTIVATION) if is_buy else 
            ...
            # Market condition adjustments
            volume_factor = min(1.0, volume / (position_value * 10)) if position_value > 0 else 0.5
            liquidity_factor = min(1.0, liquidity / (position_value * 100)) if position_value > 0 else 0.5
            volatility_factor = max(0.5, 1.0 - volatility * 0.2)
            risk_reward_ratio *= (volume_factor * liquidity_factor * volatility_factor)
            
            if risk_reward_ratio < 2.0:
                self._add_recommendation(f"Risk/reward ratio ({risk_reward_ratio:.2f}) below minimum 2.0")
                self._add_recommendation("Consider adjusting entry price or take-profit levels")
            ...
            if volatility > self.config.VOLATILITY_SCALE_THRESHOLD:
                ...
                self._add_recommendation(f"High volatility ({volatility:.2f}) - position reduced by {(1-volatility_scale)*100:.0f}%")
            if is_meme:
                adjusted_position *= 0.5  # Additional reduction for meme tokens
                self._add_recommendation("Meme coin detected - applying 50% position reduction")
            ...
            # Portfolio metrics
            existing_positions = params.get("existing_positions", [])
            total_exposure = sum(float(pos.get("amount", 0)) * float(pos.get("price", 0)) for pos in existing_positions)
            unrealized_pnl = sum(float(pos.get("unrealized_pnl", 0)) for pos in existing_positions)
            ...
            recommendations = []
            if market_impact > self.config.MAX_SLIPPAGE * 0.5:
                recommendations.append(f"High market impact detected: {market_impact:.4f}")
            ...
            if adjusted_position < position_size:
                recommendations.append(f"Position size reduced to {adjusted_position:.4f} due to risk factors")
        ...
        if liquidity < self.config.MIN_LIQUIDITY:
            ...
            if liquidity_ratio < 0.8:  # Stricter liquidity requirement
                metrics.update({
                    ...
                    "risk_level": 0.9,
                    "max_loss": max_loss,
                    "position_size": metrics.get("position_size", 0.0) * liquidity_ratio,
                    "volatility_exposure": volatility,
                    ...
                    "take_profit_levels": take_profit_levels,
                    "trailing_stop_level": trailing_stop,
                    "dynamic_position_size": adjusted_position * max(0.3, liquidity_ratio),
                    "market_impact": market_impact,
                    "expected_slippage": expected_slippage,
                    "margin_requirements": {
        ...
        correlation = await self._is_correlated(params)
        if correlation > 0.5:
            reduction_factor = 1.0 - (correlation - 0.5)  # Linear reduction from 0.5 to 1.0
            adjusted_position *= reduction_factor
            logger.info(f"Reducing position size to {reduction_factor:.2%} due to correlation: {correlation:.2f}")
            self._add_recommendation(f"Position reduced due to {correlation:.1%} correlation")
            self._add_recommendation("Consider alternative markets to reduce correlation")
        ...
        expected_returns = [
            abs(tp - price) * adjusted_position * weight * (1.0 + (0.2 if is_meme else 0))
            for tp, weight in zip(take_profit_levels, take_profit_weights)
        ]
        expected_return = sum(expected_returns)
        ...
        volatility_penalty = max(0, min(0.3, (volatility - 1.0) / 2)) if volatility > 1.0 else 0
        
        risk_reward = base_risk_reward * (2.0 + volume_bonus + liquidity_bonus - volatility_penalty)
        if is_meme:
            risk_reward *= 1.5  # More aggressive for meme coins
        
        # Log risk metrics for debugging
        logger.debug(f"Risk metrics: max_loss={max_loss}, expected_return={expected_return}, "
                    f"risk_reward={risk_reward}, market_impact={market_impact}, "
                    f"slippage={expected_slippage}, take_profit_levels={take_profit_levels}")
        
        logger.info(f"Risk metrics calculated - Risk/Reward: {risk_reward:.2f}, "
        ...
        if rate_limit_info["remaining"] < rate_limit_info["limit"] * 0.2:
            self._add_recommendation("Approaching rate limit - consider reducing request frequency")
    ...
    async def _validate_risk_metrics(self, metrics: Dict[str, Any]) -> RiskAssessment:
        ...
        try:
            ...
            dynamic_position_size = float(metrics.get("dynamic_position_size", position_size))
            ...
            if volume_scale < 1.0:
                ...
                self._add_recommendation(f"Position reduced to {volume_scale:.0%} due to volume conditions")
            ...
            if risk_reward < final_threshold:
                reasons.append(f"Risk/reward ratio ({risk_reward:.2f}) below required target {final_threshold:.1f}")
                if risk_reward < final_threshold * 0.8:  # Stricter threshold for rejection
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.85,
                        risk_level=0.7,
            ...
            if market_impact > impact_threshold * 0.8:  # Lower threshold for impact scaling
                # Calculate position reduction based on impact more aggressively
                impact_scale = max(0.1, 1.0 - (market_impact - impact_threshold * 0.8) * 2.0)
                scaled_position = position_size * impact_scale
                
                if market_impact > impact_threshold * 0.9:  # Reject before reaching max threshold
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.8,
                        risk_level=0.6,
                        ...
                        margin_requirements=metrics.get("margin_requirements", {"required": 0.0, "available": 0.0}),
                    ...
                    self._add_recommendation(f"Position reduced to {impact_scale:.0%} due to market impact")
            
            # Ultra-strict slippage validation with aggressive scaling
            expected_slippage = metrics.get("expected_slippage", 0.0)
            if expected_slippage > self.config.MAX_SLIPPAGE * 0.5:  # Much lower threshold for scaling
                slippage_scale = max(0.1, 1.0 - (expected_slippage - self.config.MAX_SLIPPAGE * 0.5) * 3.0)
                scaled_position = position_size * slippage_scale
                metrics["position_size"] = scaled_position
                metrics["dynamic_position_size"] = scaled_position
                reasons.append(f"Position reduced to {slippage_scale:.0%} due to high slippage")
                
                if expected_slippage > self.config.MAX_SLIPPAGE * 0.8:  # Reject at lower threshold
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.85,
                        risk_level=0.7,
                        ...
                        position_size=scaled_position,
                        volatility_exposure=metrics.get("volatility_exposure", 1.0),
                        expected_return=metrics.get("expected_return", 0.0) * slippage_scale,
            ...
            min_volume = position_size * price * 0.5  # Only require 0.5x position value in volume
            ...
            if volume < min_volume:
                ...
                metrics["position_size"] = scaled_position
                metrics["dynamic_position_size"] = scaled_position
                reasons.append(f"Position reduced to {volume_scale:.0%} due to low volume")
                ...
                if volume < min_volume * 0.3:  # Only reject if volume is extremely low
                    return RiskAssessment(
                        ...
                        market_impact=metrics.get("market_impact", 0.0),
                        expected_slippage=metrics.get("expected_slippage", 0.0),
                        margin_requirements=metrics.get("margin_requirements", {"required": 0.0, "maintenance": 0.0}),
                        correlation_factor=metrics.get("correlation_factor", 0.0),
                        liquidity_score=liquidity_score,
                        volume_profile=metrics.get("volume_profile", {})
                    )
            ...
            volatility_factor = max(2.0, volatility * 1.5)  # Much stricter volatility scaling
            market_factor = 1.0 / max(0.3, market_conditions)  # More aggressive market conditions scaling
            liquidity_factor = max(1.5, self.config.MIN_LIQUIDITY / max(liquidity_score, 1.0))
            spread_factor = max(1.5, expected_slippage / max(self.config.MAX_SLIPPAGE * 0.5, 0.0001))
            
            final_threshold = base_threshold * volatility_factor * market_factor * liquidity_factor * spread_factor
            
            if risk_reward_ratio < final_threshold:  # Reject if below threshold
                scaled_position = position_size * max(0.3, risk_reward_ratio / final_threshold)
                return RiskAssessment(
                    is_valid=False,
                    confidence=0.85,
                    risk_level=0.7,
                ...
                self._add_recommendation(f"Risk/reward ratio ({risk_reward_ratio:.2f}) below target {final_threshold:.1f}")
                self._add_recommendation("Consider adjusting entry price or take-profit levels")
            ...
            if volatility > volatility_threshold:
                ...
                if volatility > volatility_threshold * 1.2:  # Reject on moderately high volatility
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.85,
                        risk_level=0.8,
                    ...
                    metrics["position_size"] = scaled_position
                    metrics["dynamic_position_size"] = scaled_position
                    self._add_recommendation(f"Position reduced to {vol_scale:.0%} due to volatility")
            ...
            if correlation > 0.5:  # Lower threshold for scaling
                correlation_penalty = (correlation - 0.5) * 2.0  # More aggressive scaling
                scale = max(0.2, 1.0 - correlation_penalty)
                metrics["position_size"] *= scale
                metrics["dynamic_position_size"] = metrics["position_size"]
                reasons.append(f"Position reduced to {scale:.0%} due to high correlation")
            ...
            if correlation > 0.8:  # Lower threshold for rejection
                ...
                metrics["position_size"] = scaled_position
                metrics["dynamic_position_size"] = scaled_position
                reasons.append(f"Critical correlation ({correlation:.2f}) - position reduced to {scale:.0%}")
            ...
            if volatility_exposure > 1.5:
                ...
                if volatility_exposure > 2.0:
                    reasons.append("Warning: Extreme volatility - position size reduction recommended")
            ...
            if unrealized_pnl < -self.config.MAX_DRAWDOWN:
                ...
                scaled_position = current_position * scale
                if unrealized_pnl < -self.config.MAX_DRAWDOWN * 1.5:  # Only reject on severe drawdown
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.9,
                        risk_level=0.8,
            ...
            if volume < min_volume and position_value > 1000:  # Only check significant positions
                vol_scale = max(0.6, (volume / min_volume) ** 0.5)  # Smoother scaling
                scaled_position = current_position * vol_scale
                metrics["position_size"] = scaled_position
                metrics["dynamic_position_size"] = scaled_position
                reasons.append(f"Volume below optimal - position adjusted to {vol_scale:.0%}")
                
                if volume < min_volume * 0.1 and position_value > 5000:  # Only reject very low volume on large trades
                    return RiskAssessment(
                        is_valid=False,
                        confidence=0.85,
                        risk_level=0.7,
            ...
            if (market_alignment < 0.85 or  # Higher alignment requirement
                volatility > self.config.VOLATILITY_SCALE_THRESHOLD * 1.2 or  # Stricter volatility check
                liquidity < self.config.MIN_LIQUIDITY * 1.5 or  # Higher liquidity requirement
                spread > self.config.MAX_SLIPPAGE * 0.8 or  # Lower slippage tolerance
                not market_check.get("is_valid", False)):
                return RiskAssessment(
                    is_valid=False,
                    confidence=0.9,
                    risk_level=0.8,
            ...
            if is_meme:
                ...
                if volatility > volatility_threshold:
                    vol_scale = max(0.5, 1.0 - (volatility - volatility_threshold) * 0.15)
                    metrics["dynamic_position_size"] *= vol_scale
                    reasons.append(f"Volatility adjustment - position scaled to {vol_scale:.0%}")
                    
                    if volatility > volatility_threshold * 2.0:  # Much higher threshold for reduction
                        scale = max(0.4, 1.0 - (volatility - volatility_threshold * 2.0) * 0.3)
                        scaled_position = position_size * scale
                        reasons.append(f"High volatility - position adjusted to {scale:.0%}")
                        
                        if volatility > volatility_threshold * 3.0:  # Only reduce significantly at triple threshold
                            return RiskAssessment(
                                is_valid=False,
                                confidence=0.9,
                                risk_level=0.8,
            ...
            if (metrics.get("volatility_exposure", 1.0) > self.config.VOLATILITY_SCALE_THRESHOLD * 1.5 or
                metrics.get("market_impact", 0.0) > self.config.MAX_SLIPPAGE * 0.7 or
                metrics.get("expected_slippage", 0.0) > self.config.MAX_SLIPPAGE * 0.7 or
                metrics.get("liquidity_score", 0.0) < self.config.MIN_LIQUIDITY * 1.2 or
                metrics.get("correlation_factor", 0.0) > 0.75):
                return RiskAssessment(
                    is_valid=False,
                    confidence=0.95,
                    risk_level=0.9,
            ...
            risk_scale = min(
                1.0,
                (self.config.VOLATILITY_SCALE_THRESHOLD * 1.5) / max(metrics.get("volatility_exposure", 1.0), 0.1),
                (self.config.MAX_SLIPPAGE * 0.7) / max(metrics.get("market_impact", 0.001), 0.001),
                (self.config.MAX_SLIPPAGE * 0.7) / max(metrics.get("expected_slippage", 0.001), 0.001),
            ...
            if margin_ratio > 0.6:  # Start scaling down at 60% utilization
                ...
                reasons.append(f"Position reduced to {margin_scale:.0%} due to high margin usage")
            ...
            final_risk_level = max(
                0.5,  # Base risk level
                metrics.get("volatility_exposure", 1.0) / self.config.VOLATILITY_SCALE_THRESHOLD,
                metrics.get("market_impact", 0.0) / (self.config.MAX_SLIPPAGE * 0.5),
                metrics.get("expected_slippage", 0.0) / (self.config.MAX_SLIPPAGE * 0.5),
                metrics.get("correlation_factor", 0.0) * 1.2,
                1.0 - (metrics.get("liquidity_score", 0.0) / (self.config.MIN_LIQUIDITY * 1.5))
            )
            
            # Stricter position size adjustment
            final_scale = min(
                risk_scale,
                1.0 - (final_risk_level - 0.5) * 1.5,  # More aggressive scaling based on risk
            ...
            if final_risk_level > 0.7:
                final_recommendations.append("Warning: High combined risk level - exercise caution")
            if final_scale < 0.5:
                final_recommendations.append("Significant position reduction due to multiple risk factors")
            ...
            return RiskAssessment(
                ...
                rate_limit_info={"is_limited": False, "remaining": 100, "reset": current_time + 60}
            )
    ...
    async def _is_correlated(self, params: Dict[str, Any]) -> float:
        ...
        for position in existing_positions:
            ...
            pos_value = float(position.get("amount", 0)) * float(position.get("price", 0))
    ...
    async def _generate_recommendations(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        risk_reward = metrics.get("risk_reward_ratio", 0.0)
        if risk_reward < 1.5:
            recommendations.append(f"Risk-reward ratio ({risk_reward:.2f}) below target - consider adjusting entry")
        ...
        if market_impact > 0.01:
            recommendations.append(f"High market impact ({market_impact:.3f}) - consider splitting order")
        ...
        if volume_profile and "volume" in volume_profile:
            ...
            if relative_volume < 0.5:
                recommendations.append("Low volume relative to liquidity - monitor execution")
                if relative_volume < 0.2:
                    recommendations.append("Consider reducing order size due to low liquidity")
        
        # Volatility recommendations
        volatility = metrics.get("volatility_exposure", 1.0)
        if volatility > 1.5:
            recommendations.append(f"High volatility ({volatility:.2f}) - consider reducing position size")
            if volatility > 2.0:
                recommendations.append("Use multiple take-profit levels in high volatility")
        
        # Correlation recommendations
        correlation = metrics.get("correlation_factor", 0.0)
        if correlation > 0.7:
            recommendations.append("High correlation with existing positions - consider reducing exposure")
            if correlation > 0.9:
        ...
        if margin_requirements:
            ...
            if margin_used > 0.7:
                recommendations.append(f"High margin utilization ({margin_used:.1%}) - monitor closely")
                if margin_used > 0.9:
                    recommendations.append("Critical margin level - consider reducing exposure")
        
        # Meme coin recommendations
        if volume_profile and volume_profile.get("is_meme_coin", False):
            recommendations.append("Meme coin detected - use extra caution and reduced position size")
        ...
        if take_profit_levels:
            ...
            tp_str = ', '.join(f'{tp:.0f}' if tp >= 100 else f'{(tp-1)*100:.1f}%' for tp in tp_levels)
            recommendations.append(f"Recommended take-profit levels: {tp_str}")
            recommendations.append("Consider using multiple take-profit levels to maximize gains")
        ...
        if liquidity_score < self.config.MIN_LIQUIDITY:
            recommendations.append(f"Low liquidity ({liquidity_score:.0f}) - consider reducing order size")
            if liquidity_score < self.config.MIN_LIQUIDITY * 0.5:
                recommendations.append("Critical liquidity level - consider alternative markets")
        
        # Slippage recommendations
        slippage = metrics.get("expected_slippage", 0.0)
        if slippage > 0.01:
            recommendations.append(f"High expected slippage ({slippage:.1%}) - use limit orders")
            if slippage > 0.02:
                recommendations.append("Consider splitting order to reduce slippage impact")

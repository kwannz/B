...
class FallbackManager(Generic[T, R]):
    def __init__(self, new_system: Any, legacy_system: Any, timeout: float = None, 
                 handled_exceptions: tuple = None, max_retries: int = 0):
        self.new = new_system
    ...
    async def execute(self, request: T) -> Optional[R]:
        async with self._lock:
            for retry in range(self.max_retries + 1):
                try:
                    if self.timeout:
                        result = await asyncio.wait_for(self._try_process(self.new, request), self.timeout)
                    else:
                    ...
                    logging.warning(f"Primary system timed out (attempt {retry + 1}/{self.max_retries + 1})")
                    if retry == self.max_retries:
                        track_fallback_rate(success=False)
                        break
                    continue
                except Exception as e:
                    if not isinstance(e, self.handled_exceptions) or retry == self.max_retries:
                        track_fallback_rate(success=False)
                        break
                    logging.warning(f"Primary system failed (attempt {retry + 1}/{self.max_retries + 1}): {str(e)}")
                    continue
            ...
            try:
                if self.timeout:
                    result = await asyncio.wait_for(self._try_process(self.old, request), self.timeout)
                else:

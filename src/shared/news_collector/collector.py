...
class NewsCollector:
    ...
    async def _fetch_news(self, source: str, url: str, cutoff_time: datetime) -> List[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                self.session._default_headers['User-Agent'] = self._get_random_user_agent()
                
                async with self.session.get(url, allow_redirects=True, ssl=False, timeout=30) as response:
                    if response.status == 200:
                        ...
                        articles = []
                        selectors = {
                            'bloomberg': {'article': 'article.story-package-module__story', 'title': 'h3.story-package-module__headline', 'link': 'a.story-package-module__headline__link', 'time': 'time.story-package-module__timestamp'},
                            'reuters': {'article': 'article.media-story-card', 'title': 'h3.media-story-card__heading__eqhp9', 'link': 'a.media-story-card__heading__eqhp9', 'time': 'time.media-story-card__datetime__eqhp9'},
                            'cryptoslate': {'article': 'article.post-card', 'title': 'h2.post-title', 'link': 'a.post-title-link', 'time': 'time.post-date'},
                            'ambcrypto': {'article': 'article.jeg_post', 'title': 'h3.jeg_post_title', 'link': 'a.jeg_post_title', 'time': 'div.jeg_meta_date'},
                            'theblockbeats': {'article': 'div.news-item', 'title': 'h3.news-title', 'link': 'a.news-link', 'time': 'span.time'},
                            'chaincatcher': {'article': 'div.article-item', 'title': 'h3.article-title', 'link': 'a.article-link', 'time': 'span.time'},
                            'foresightnews': {'article': 'div.news-item', 'title': 'h3.news-title', 'link': 'a.news-link', 'time': 'span.time'},
                            'binance_cn': {'article': 'div.css-1wr4jig', 'title': 'h3.css-1nqr3i7', 'link': 'a.css-1nqr3i7', 'time': 'span.css-1gqo2zd'},
                            'odaily': {'article': 'div.article-item', 'title': 'h3.article-title', 'link': 'a.article-link', 'time': 'span.time'},
                            'jinse': {'article': 'div.content-item', 'title': 'h3.content-title', 'link': 'a.content-link', 'time': 'span.time'},
                            'coindesk': {'article': 'div.article-cardstyles__StyledWrapper-sc-q1x8lc-0', 'title': 'h6.typography__StyledTypography-sc-owin6q-0', 'link': 'a.card-title', 'time': 'span.date-modified'},
                            'cointelegraph': {'article': 'article.post-card-inline', 'title': 'span.post-card-inline__title', 'link': 'a.post-card-inline__title-link', 'time': 'time.post-card-inline__date'},
                            'decrypt': {'article': 'article.storyCard', 'title': 'h3.heading', 'link': 'a.storyCard__link', 'time': 'time.timestamp'}
                        }.get(source, {'article': 'article', 'title': 'h2,h3', 'link': 'a', 'time': 'time'})
                        ...
                        for article in soup.select(selectors['article'])[:10]:
                            ...
                            timestamp = self._parse_timestamp(time_elem.get_text(strip=True)) if time_elem else datetime.now()
                            ...
                            if timestamp >= cutoff_time:
                                articles.append({
                                    ...
                                    'timestamp': timestamp,
                                    'content': '',
                                    'language': 'zh' if source in self.chinese_sources else 'en'
                                })
                        
                        return articles
                    
                    elif response.status in [403, 401]:
                        print(f"Access denied for {source}, attempt {attempt + 1}/{max_retries}")
    ...
    async def _fetch_telegram_news(self, url: str, cutoff_time: datetime) -> List[Dict[str, Any]]:
    ...
    async def process_source(self, url: str, name: str, parser: Callable[[str], Union[List[Dict[str, Any]], Awaitable[List[Dict[str, Any]]]]]) -> None:
                ...
                try:
                    if isinstance(article, dict):
                        raw_article = RawNewsArticle(
                            ...
                            analysis_metadata={'language': article.get('language', 'en')}
                        )
                    else:
                        raw_article = article
                    ...
                    analysis = SentimentAnalysis(
                        ...
                        metadata={'language': raw_article.analysis_metadata.get('language', 'en')}
                    )

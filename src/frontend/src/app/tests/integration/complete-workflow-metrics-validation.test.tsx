...
describe('Complete Workflow Metrics Validation', () => {
  ...
  it('should validate complete workflow with metrics collection and error handling', async () => {
    await testRunner.runTest(async () => {
      ...
      const workflowMetrics = {
        ...
        errors: {
          ...
          steps: errorData.map(e => e.step),
          recovery_attempts: errorData.map(e => ({
            step: e.step,
        ...
        system: {
          final_heap_used: metricsData[metricsData.length - 1]?.system.heap_used ?? 0,
      ...
      expect(workflowMetrics.errors.count).toBe(0);
    });
  });

  it('should validate AB wallet comparison with system metrics and error recovery', async () => {
    await testRunner.runTest(async () => {
      const walletA = {
      ...
      const comparisonMetrics = {
        ...
        errors: {
          ...
          error_metrics: errorMetrics.map(e => ({
            timestamp: e.timestamp,
            metrics: e.metrics
          }))
        },
      ...
      expect(comparisonMetrics.duration).toBeLessThan(5000);

...
export const setupTestEnvironment = () => {
  ...
  const mockSuccessfulAPI = () => {
    mockAPI.createWallet.mockResolvedValue(createMockApiResponse({
      address: 'test-wallet',
    ...
    mockAPI.getWallet.mockResolvedValue(createMockApiResponse({
      address: 'test-wallet',
      private_key: 'test-key',
      balance: 0,
      bot_id: 'test-bot'
    ...
    mockAPI.createBot.mockResolvedValue(createMockApiResponse({
      id: 'test-bot',
      type: 'trading',
    ...
    mockAPI.getBotStatus.mockResolvedValue(createMockApiResponse({
      id: 'test-bot',
      status: 'active'
    }));

    mockAPI.transferSOL.mockResolvedValue(createMockApiResponse({
  ...
  const mockSlowAPI = () => {
    ...
    Object.values(mockAPI).forEach(mock => {
      mock.mockImplementation(() => new Promise(resolve => 
        setTimeout(() => resolve(createMockApiResponse({})), delay)
      ));
    });
  };

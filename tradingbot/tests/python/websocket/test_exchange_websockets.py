...
from tradingbot.trading_agent.python.exchanges.pancakeswap_ws import PancakeSwapWebSocket
...
async def test_websocket_reconnection():
    ...
    with patch("aiohttp.ClientSession.ws_connect", side_effect=mock_connect):
        ...
        # 第一次连接失败
        await ws.initialize()
        assert ws.initialized is False
...
async def test_websocket_error_handling(mock_monitor):
    ...
    # 模拟错误情况
    error_data = {"s": "BTCUSDT", "error": "Invalid message"}
    ...
    mock_monitor.record_request.assert_called_with(
        ...
        response_time=mock_monitor.record_request.call_args[1]["response_time"],
        is_error=True,
    )

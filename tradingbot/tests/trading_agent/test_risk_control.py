...
async def test_check_drawdown_limits(risk_controller):
    ...
    result = await risk_controller.check_drawdown_limits(portfolio_history, max_drawdown_limit)
...
async def test_check_exposure_limits(risk_controller):
    ...
    assert all(exp <= max_exposure_per_asset for exp in result['asset_exposures'].values())

...
def mock_db_session(monkeypatch):
    ...
    monkeypatch.setattr('tradingbot.src.trading_agent.api.worker.get_db', lambda: db_gen)
    return session
...
def test_update_positions(mock_db_session, mock_strategy):
    ...
    with patch('tradingbot.src.trading_agent.api.worker.get_strategy_instance') as mock_get_strategy:
...
def test_check_strategy_health(mock_db_session):
    ...
    trade_query = mock_db_session.query.return_value
    trade_query.filter.return_value.order_by.return_value.first.return_value = recent_trades[0]
...
def test_celery_beat_schedule():
    ...
    positions_schedule = celery.conf.beat_schedule["update-positions-every-minute"]["schedule"]
    assert positions_schedule.minute == "*"
    
    metrics_schedule = celery.conf.beat_schedule["update-metrics-every-hour"]["schedule"]
    assert metrics_schedule.hour == "*"
    
    health_schedule = celery.conf.beat_schedule["check-strategy-health-every-5-minutes"]["schedule"]

...
async def mock_client():
    ...
    with patch("solana.rpc.async_api.AsyncClient") as mock:
        ...
        # Mock get_balance
        client.get_balance = AsyncMock(return_value={"result": {"value": int(1e9)}})  # 1 SOL
...
async def test_create_wallet(wallet_manager):
    ...
    assert len(base58.b58decode(wallet["private_key"])) == 64  # Solana private key length
...
async def test_transfer_tokens(wallet_manager, mock_client):
    ...
    result = await wallet_manager.transfer_tokens(
        TEST_WALLET_A_PRIVATE_KEY,
        TEST_WALLET_B_ADDRESS,
        0.5  # 0.5 SOL
    )
    assert result is not None
    assert result["signature"] == "test_signature"
    assert result["from_address"] == str(Keypair.from_secret_key(base58.b58decode(TEST_WALLET_A_PRIVATE_KEY)).public_key)
...
async def test_monitor_balance(wallet_manager):
    ...
    with patch.object(wallet_manager, "get_balance") as mock_balance:
        mock_balance.side_effect = [1.0, 2.0]  # First call returns 1.0, second call returns 2.0
...
async def test_verify_wallet(wallet_manager, mock_client):
    ...
    assert await wallet_manager.verify_wallet("valid_address", min_balance=0.5)  # Should pass (mock returns 1.0 SOL)
    assert not await wallet_manager.verify_wallet("valid_address", min_balance=2.0)  # Should fail (mock returns 1.0 SOL)
...
async def test_transfer_tokens_reserves_fee():
    ...
    # Mock dependencies
    with patch.object(
        manager, "get_balance", new_callable=AsyncMock

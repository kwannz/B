...
def mock_env_and_config():
    with patch.dict(os.environ, {'DEEPSEEK_API_KEY': 'test_key'}), \
         ...
         patch('src.trading_agent.models.manager.DatabaseManager', return_value=MockDatabaseManager()), \
...
async def test_analyze_market_includes_promoted_words():
    ...
    mock_session = AsyncMock()
    mock_session.close = AsyncMock()
    mock_session.post = AsyncMock(return_value=create_mock_response(
        status=200,
        body={
            "choices": [
                {

...
async def test_market_cap_threshold(strategy_config, mock_market_data, mock_market_data_aggregator):
...
async def test_liquidity_threshold(strategy_config, mock_market_data, mock_market_data_aggregator):
...
async def test_volume_threshold(strategy_config, mock_market_data, mock_market_data_aggregator):
...
async def test_age_threshold(strategy_config, mock_market_data, mock_market_data_aggregator):
...
async def test_confidence_calculation(strategy_config, mock_market_data, mock_market_data_aggregator):
...
async def test_trade_execution(
    ...
    test_tenant, test_wallet, test_strategy
):
...
async def test_position_management(
    ...
    with get_tenant_session() as session:
        ...
        mock_market_data[0]["price"] = trade_data["price"] * (1 + strategy.profit_target + 0.01)
        await strategy.update_positions(
            tenant_id=test_tenant.id,
            market_data=mock_market_data[0]
        )
        ...
        mock_market_data[0]["price"] = trade_data["price"] * (1 - strategy.stop_loss - 0.01)
        await strategy.update_positions(
            tenant_id=test_tenant.id,
            market_data=mock_market_data[0]
        )
        ...
        assert "market_cap_increase" in another_trade.trade_metadata.get("exit_reason", "")

        session.commit()


async def test_error_handling(strategy_config, mock_market_data, mock_market_data_aggregator):

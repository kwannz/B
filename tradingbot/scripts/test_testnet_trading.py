    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            params = {
                ...
                "slippageBps": str(int(TEST_CONFIG["slippage"] * 100)),  # Convert to string
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                        ...
                        if route_plan:
                            ...
                            for i, step in enumerate(route_plan):
                                ...
                                if not isinstance(swap_info, dict):
                                    logger.warning(f"Invalid swap info format in step {i + 1}")
                                    continue
                                ...
                                try:
                                    ...
                                    logger.info(f"\nStep {i + 1}:")
                                    logger.info(f"  DEX: {swap_info.get('label', 'Unknown')}")
                                    logger.info(f"  Input Amount: {in_amount/1e9:.6f} SOL")
                                    logger.info(f"  Output Amount: {out_amount/1e6:.6f} USDC")
                                    logger.info(f"  Fee Amount: {fee_amount/1e9:.6f} SOL")
                                    
                                    total_input_amount += in_amount
                                    total_output_amount += out_amount
                                    total_fees += fee_amount
                                except (ValueError, TypeError) as e:
                                    logger.warning(f"Error parsing amounts in step {i + 1}: {str(e)}")
                            
                            # Log totals
                            logger.info("\nRoute Summary:")
                            logger.info(f"Total Input: {total_input_amount/1e9:.6f} SOL")
                            logger.info(f"Total Output: {total_output_amount/1e6:.6f} USDC")
                            logger.info(f"Total Fees: {total_fees/1e9:.6f} SOL")
                            
                            # Calculate effective price
                            if total_input_amount > 0:
                                effective_price = (total_output_amount / 1e6) / (total_input_amount / 1e9)
                                logger.info(f"Effective Price: ${effective_price:.2f} per SOL")
                            ...
                            if price_impact is not None:
                                try:
                                    ...
                                    logger.warning(f"Invalid price impact format: {price_impact}")
                    ...
                    error_text = await response.text()
                    logger.error(f"Route API failed with status {response.status}: {error_text}")
                return False
    except Exception as e:
        logger.error(f"Route aggregation test failed: {str(e)}", exc_info=True)
        return False

async def wait_for_confirmation(client: AsyncClient, signature: str, max_retries: int = 30) -> bool:
...
async def main():
    ...
    success = all(results)
    logger.info(f"Testnet trading tests {'completed successfully' if success else 'failed'}")
    return success

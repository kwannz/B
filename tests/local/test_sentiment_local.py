...
async def test_local_model_priority():
    ...
    mock_local_response = {"response": json.dumps({"score": 0.9, "label": "positive"})}
    ...
    with (
        ...
        mock_context.__aenter__.return_value.json = AsyncMock(
            return_value=mock_local_response
        )
...
async def test_remote_fallback():
    ...
    mock_remote_response = {"choices": [{"score": 0.5}]}

    with (
        patch("src.shared.sentiment.sentiment_analyzer.AI_MODEL_MODE", "REMOTE"),
        patch("aiohttp.ClientSession.post") as mock_post,
    ):
        mock_context = AsyncMock()
        mock_context.__aenter__.return_value.status = 200
        mock_context.__aenter__.return_value.json = AsyncMock(
            return_value=mock_remote_response
        )
        mock_post.return_value = mock_context
...
async def test_chinese_text():
    ...
    mock_response = {"response": json.dumps({"score": 0.95, "label": "positive"})}

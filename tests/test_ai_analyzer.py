...
async def test_validate_trade_with_r1_model(mock_deepseek_api):
    ...
    try:
        ...
        assert all(k in risk for k in ["risk_level", "max_loss", "position_size", "volatility_exposure"])
        ...
        assert all(k in metrics for k in ["expected_return", "risk_reward_ratio", "market_conditions_alignment"])
...
async def test_validate_trade_error_handling(mock_deepseek_api):
    ...
    try:
        ...
        with pytest.raises(ValueError):
            await analyzer.validate_trade(
                {"id": "test", "type": "market", "amount": 1.0},
                {"invalid": "data"}
            )

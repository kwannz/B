...
async def test_risk_metrics(risk_manager):
    ...
    assert assessment.dynamic_position_size == trade["amount"]  # No reduction for low volatility
...
async def test_recommendations(risk_manager):
    ...
    assert any("multiple take-profit levels" in rec.lower() for rec in assessment.recommendations)
...
async def test_config_validation():
    ...
    with pytest.raises(ValueError, match="DYNAMIC_TAKE_PROFIT_LEVELS must be in ascending order"):
        config.validate()
    ...
    with pytest.raises(ValueError, match="POSITION_REDUCTION_RATE must be between 0 and 1"):
        config.validate()
    ...
    with pytest.raises(ValueError, match="TRAILING_STOP_ACTIVATION must be between 0 and 3"):
        config.validate()
...
async def test_high_volatility(risk_manager):
    trade = create_trade_dict(
        amount=2,
        volatility=2.2,  # High volatility
        liquidity=1000000,
        volume=10000
    )
    ...
    assessment = await risk_manager.assess_trade(trade)
    assert assessment.is_valid
    assert assessment.dynamic_position_size < trade["amount"]  # Position size should be reduced
    assert len(assessment.take_profit_levels) == 3
    assert assessment.trailing_stop_level > trade["price"]
    assert any("volatility" in rec.lower() for rec in assessment.recommendations)
...
async def test_concurrent_trades(risk_manager):
    ...
    trade1 = create_trade_dict(
        amount=1,
        price=50000
    )
    trade2 = create_trade_dict(
        amount=0.5,
        price=50100
    )
    ...
    assert all(len(a.take_profit_levels) == 3 for a in [assessment1, assessment2, assessment3, assessment4])
    assert all(a.trailing_stop_level is not None for a in [assessment1, assessment2, assessment3, assessment4])
...
async def test_meme_coin_risk(risk_manager):
    ...
    assert assessment.is_valid
    assert assessment.dynamic_position_size <= meme_trade["amount"] * 0.5  # 50% reduction
    assert len(assessment.take_profit_levels) == 3
...
async def test_correlation_based_sizing(risk_manager):
    ...
    assert assessment1.is_valid
    assert assessment1.dynamic_position_size < btc_trade["amount"]  # Should reduce size due to correlation
    ...
    assert assessment3.is_valid
    assert assessment3.dynamic_position_size <= doge_trade["amount"] * 0.5  # Meme coin reduction
...
async def test_drawdown_limits(risk_manager):
    ...
    eth_position.update({
        "margin_used": 15000,
        "unrealized_pnl": -8000  # High unrealized loss
    })
    trade = create_trade_dict(
        existing_positions=[eth_position]
    )
    ...
    high_drawdown_trade["existing_positions"][0]["unrealized_pnl"] = -15000  # 15% drawdown
...
async def test_take_profit_and_trailing_stop(risk_manager):
    ...
    assert all(tp2 > tp1 for tp1, tp2 in zip(assessment.take_profit_levels, assessment2.take_profit_levels))
    ...
    assert assessment3.is_valid
    assert assessment3.trailing_stop_level > profit_trade["price"] * 1.45  # Higher trailing stop in profit
    ...
    assert all(tp4 > tp1 for tp1, tp4 in zip(assessment.take_profit_levels, assessment4.take_profit_levels))
    assert len([rec for rec in assessment4.recommendations if any(
...
async def test_market_impact_assessment(risk_manager):
    ...
    assert len([rec for rec in assessment4.recommendations if any(
        x in rec.lower() for x in ["market impact", "slippage", "volatility"]
    )]) >= 2
...
async def test_dynamic_risk_adjustment(risk_manager):
    ...
    assert assessment2.dynamic_position_size <= base_trade["amount"] * 0.7  # POSITION_REDUCTION_RATE
    assert any("volatility" in rec.lower() for rec in assessment2.recommendations)
    ...
    assert len([rec for rec in assessment4.recommendations if any(
        x in rec.lower() for x in ["volatility", "spread", "correlation", "liquidity"]
    )]) >= 3
...
async def test_portfolio_risk_and_margin(risk_manager):
    ...
    assessment1 = await risk_manager.assess_trade(base_trade)
    assert assessment1.is_valid
    assert assessment1.margin_requirements["required"] <= base_trade["account_size"] * 0.2
    ...
    assessment2 = await risk_manager.assess_trade(create_trade_dict(
        amount=3,
        existing_positions=[eth_position]
    ))
    assert assessment2.is_valid
    assert any("margin utilization" in rec.lower() for rec in assessment2.recommendations)
    ...
    assessment3 = await risk_manager.assess_trade(create_trade_dict(
        amount=5,
        existing_positions=[eth_position]
    ))
    assert not assessment3.is_valid
    ...
    assessment1 = await risk_manager.assess_trade(create_trade_dict(
        volatility=2.5,
    ...
    # Test combined risk factors
    assessment4 = await risk_manager.assess_trade(create_trade_dict(
        amount=5.0,
        price=50000,
        volatility=2.0,
        liquidity=750000,
    ...
    assert len([rec for rec in assessment4.recommendations if any(
        x in rec.lower() for x in ["volatility", "liquidity", "spread", "correlation"]
    )]) >= 3
    
    # Test drawdown scenarios
    eth_position["unrealized_pnl"] = -8000  # 8% drawdown
    assessment5 = await risk_manager.assess_trade(create_trade_dict(
        existing_positions=[eth_position]
    ))
    assert assessment5.is_valid
    ...
    assessment6 = await risk_manager.assess_trade(create_trade_dict(
        existing_positions=[eth_position]
    ))
    ...
    assert assessment2.is_valid
    assert assessment2.dynamic_position_size < assessment1.dynamic_position_size
    assert any("existing position" in rec.lower() for rec in assessment2.recommendations)

...
def test_create_access_token():
    ...
    empty_payload = jwt.decode(
        empty_token, settings.JWT_SECRET, algorithms=[settings.JWT_ALGORITHM]
    )
...
async def test_get_current_user():
    ...
    expired_token = create_access_token(
        {"sub": "testuser"}, expires_delta=timedelta(microseconds=1)
    )
...
def test_extract_request_context(
    mock_request, mock_request_no_client, mock_request_no_user_agent
):
...
def test_token_models():
    ...
    assert token_dict == {"access_token": "test_token", "token_type": "bearer"}

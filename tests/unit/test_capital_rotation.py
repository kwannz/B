...
def test_init_invalid_performance_window():
    ...
    config = StrategyConfig(
        strategy_type="capital_rotation",
        parameters={"performance_window": -1}
    )
    with pytest.raises(ValueError, match="performance_window must be a positive integer"):
...
def test_init_invalid_rotation_interval():
    ...
    config = StrategyConfig(
        strategy_type="capital_rotation",
        parameters={"rotation_interval": 0}
    )
    with pytest.raises(ValueError, match="rotation_interval must be a positive integer"):
        CapitalRotationStrategy(config)
...
def test_init_invalid_num_top_assets():
    ...
    config = StrategyConfig(
        strategy_type="capital_rotation",
        parameters={"num_top_assets": "invalid"}
    )
...
async def test_calculate_signals_error(valid_config):
    ...
    strategy = CapitalRotationStrategy(valid_config)
    market_data = [{"invalid": "data"}]  # This will cause an error in _calculate_relative_strength
    result = await strategy.calculate_signals(market_data)
    assert result["signal"] == "neutral"
    assert result["confidence"] == 0.0

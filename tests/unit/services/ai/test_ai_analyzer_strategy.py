...
def mock_strategy_response():
    return {
        "strategy": {
            ...
            "indicators": {"rsi_period": 14, "ma_fast": 10, "ma_slow": 21},
            "timeframe": "1h",
        }
    }
...
async def test_optimize_strategy_parameters():
    strategy = {
        "type": "mean_reversion",
        "parameters": {"window": 10, "entry_threshold": 2.0},
    }
    historical_data = [
        {"price": 100, "volume": 1000, "timestamp": "2024-01-01"},
...
async def test_validate_strategy():
    invalid_strategy = {"type": "invalid_type", "parameters": {}}
    async with get_analyzer() as analyzer:
...
async def test_adapt_strategy():
    strategy = {
        ...
        "parameters": {"trend_period": 30, "entry_threshold": 0.01},
    }
    market_changes = {
...
async def test_combine_strategies():
    strategies = [
        {"type": "momentum", "weight": 0.6, "parameters": {"lookback": 20}},
        {"type": "mean_reversion", "weight": 0.4, "parameters": {"window": 10}},
    ]
    async with get_analyzer() as analyzer:
...
async def test_backtest_strategy():
    strategy = {"type": "momentum", "parameters": {"lookback": 20, "threshold": 0.02}}
    historical_data = [
        {"price": 100, "volume": 1000, "timestamp": "2024-01-01"},
        {"price": 102, "volume": 1100, "timestamp": "2024-01-02"},

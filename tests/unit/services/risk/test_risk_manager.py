...
async def test_check_trade_validation(risk_manager):
    """测试交易验证"""
    # 测试缺少必需字段
    invalid_trade = {"price": 100, "amount": 1}
...
async def test_check_trade_limits(risk_manager):
    ...
    # 有效交易
    valid_trade = {"price": 100, "amount": 1, "type": "buy", "portfolio_value": 10000}
...
async def test_risk_level_adjustment(risk_manager):
    """测试风险等级调整"""
    # 测试高收益低波动场景
    high_performance = {"daily_return": 0.02, "volatility": 0.05, "sharpe_ratio": 2.5}
    await risk_manager.adjust_risk_level(high_performance)
    assert risk_manager.risk_level == "high"

    # 测试低收益高波动场景
    low_performance = {"daily_return": -0.01, "volatility": 0.25, "sharpe_ratio": 0.5}

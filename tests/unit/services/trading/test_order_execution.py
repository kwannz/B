...
async def test_execute_market_order(order_executor, mock_exchange):
    ...
    mock_exchange.create_order.assert_called_once_with(
        symbol="BTC/USDT", type="market", side="buy", amount=1.0
    )
...
async def test_execute_limit_order(order_executor, mock_exchange):
    ...
    mock_exchange.create_order.assert_called_once_with(
        symbol="BTC/USDT", type="limit", side="sell", amount=0.5, price=21000
    )


@pytest.mark.asyncio
async def test_execute_order_with_retry(order_executor, mock_exchange):
    order = {"symbol": "BTC/USDT", "type": "market", "side": "buy", "amount": 1.0}

    # Simulate first attempt failing, second succeeding
    mock_exchange.create_order.side_effect = [
        Exception("Network error"),
        {"id": "123458", "status": "filled", "filled": 1.0, "average": 20000},
    ]
...
async def test_cancel_order(order_executor, mock_exchange):
    ...
    mock_exchange.cancel_order.return_value = {"id": order_id, "status": "canceled"}
    ...
    mock_exchange.cancel_order.assert_called_once_with(id=order_id, symbol="BTC/USDT")


@pytest.mark.asyncio
async def test_execute_batch_orders(order_executor, mock_exchange):
    orders = [
        {"symbol": "BTC/USDT", "type": "market", "side": "buy", "amount": 0.5},
        {
            "symbol": "ETH/USDT",
            "type": "limit",
            "side": "sell",
    ...
    mock_exchange.create_order.side_effect = [
        {"id": "123460", "status": "filled", "filled": 0.5, "average": 20000},
        {"id": "123461", "status": "open", "filled": 0, "price": 1500},
    ]
...
async def test_handle_partial_fills(order_executor, mock_exchange):
    ...
    result = await order_executor.execute_order(
        order, mock_exchange, wait_for_fill=True
    )
...
async def test_validate_order(order_executor):
    ...
    invalid_order = {"symbol": "BTC/USDT", "type": "limit"}

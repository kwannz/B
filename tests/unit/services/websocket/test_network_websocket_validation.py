    ...
    with patch("aiohttp.ClientSession.ws_connect", return_value=mock_ws):
        ...
        assert (
            metrics.websocket_errors.labels(
                path="ws://test.com/ws/market", error_type="json_decode"
            )._value.get()
            >= 1
        )
...
async def test_websocket_validation_connection_error():
    ...
    with patch("aiohttp.ClientSession.ws_connect", return_value=mock_ws):
        ...
        assert (
            metrics.websocket_errors.labels(
                path="ws://test.com/ws/market", error_type="connection_error"
            )._value.get()
            >= 1
        )
    ...
    with patch("aiohttp.ClientSession.ws_connect", return_value=mock_ws):
        ...
        assert (
            metrics.websocket_messages.labels(
                path="ws://test.com/ws/market", type="text"
            )._value.get()
            == 2
        )
    ...
    with patch("aiohttp.ClientSession.ws_connect", return_value=mock_ws):
        ...
        assert (
            metrics.websocket_connections.labels(
                path="ws://test.com/ws/market", status="disconnected"
            )._value.get()
            >= 1
        )

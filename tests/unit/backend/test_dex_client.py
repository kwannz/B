...
async def test_get_quote():
    ...
    # Test Uniswap quote
    result = await client.get_quote("uniswap", "ETH", "USDC", 1.0)
    assert isinstance(result, dict)

    # Test Jupiter quote
    result = await client.get_quote("jupiter", "SOL", "USDC", 10.0)
    assert isinstance(result, dict)
...
async def test_get_liquidity():
    ...
    result = await client.get_liquidity("raydium", "SOL")
    assert isinstance(result, dict)

    # Test PancakeSwap liquidity
    result = await client.get_liquidity("pancakeswap", "CAKE")
    assert isinstance(result, dict)
...
async def test_error_handling():
    ...
    # Test invalid DEX
    result = await client.get_quote("invalid_dex", "ETH", "USDC", 1.0)
    assert "error" in result

    # Test invalid token
    result = await client.get_liquidity("uniswap", "INVALID_TOKEN")
    assert "error" in result

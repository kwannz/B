...
async def test_user_strategy_trade_relationship(db_session: AsyncSession):
    ...
    async with db_session.begin():
        ...
        assert any(
            trade.symbol == "BTC/USD" for trade in loaded_user.strategies[0].trades
        )
        assert any(
            trade.symbol == "ETH/USD" for trade in loaded_user.strategies[0].trades
        )

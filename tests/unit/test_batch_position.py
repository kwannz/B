...
def test_init_invalid_batch_targets():
    ...
    with pytest.raises(ValueError, match="Total batch target percentages cannot exceed 100%"):
...
def test_init_invalid_stop_loss():
    ...
    config = StrategyConfig(
        strategy_type="batch_position",
        parameters={
            "batch_targets": [{"percentage": Decimal("0.3"), "multiplier": Decimal("1.5")}],
...
def test_init_invalid_trailing_stop():
    ...
    with pytest.raises(ValueError, match="Trailing stop percentage must be between 0 and 1"):
        BatchPositionStrategy(config)
...
async def test_calculate_signals_valid_data(valid_config):
    ...
    strategy = BatchPositionStrategy(valid_config)
    market_data = [
        {"price": Decimal("100"), "volume": Decimal("1000")}
    ]

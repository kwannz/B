...
from tradingbot.shared.models.trading import TradeStatus, OrderType, OrderSide
...
async def test_analyze_market_conditions_success(trading_agent):
    ...
    with patch.object(trading_agent, 'analyze_text', new_callable=AsyncMock) as mock_analyze:
...
async def test_analyze_market_conditions_error(trading_agent):
    ...
    with patch.object(trading_agent, 'analyze_text', side_effect=Exception("Analysis failed")):
        result = await trading_agent._analyze_market_conditions("SOL")
        assert result == {}
...
async def test_generate_strategy_json_error(trading_agent):
    ...
    mock_response = {
        "choices": [{
            "text": "Invalid JSON"  # This will cause json.loads to fail
        }]
    }
...
async def test_start_insufficient_balance(trading_agent):
    ...
    with patch.object(trading_agent, 'get_wallet_balance', new_callable=AsyncMock) as mock_balance:
...
async def test_start_success(trading_agent):
    ...
    with patch.multiple(trading_agent,
...
async def test_start_strategy_generation_error(trading_agent):
    ...
    with patch.multiple(trading_agent,
                       get_wallet_balance=AsyncMock(return_value=1.0),
                       _generate_strategy=AsyncMock(side_effect=Exception("Strategy generation failed"))):
...
async def test_get_wallet_balance_success(trading_agent):
    ...
    with patch.object(trading_agent.wallet, 'get_balance', new_callable=AsyncMock) as mock_balance:
...
async def test_get_wallet_balance_error(trading_agent):
    ...
    with patch.object(trading_agent.wallet, 'get_balance', side_effect=Exception("Balance error")):
        balance = await trading_agent.get_wallet_balance()
        assert balance == 0.0

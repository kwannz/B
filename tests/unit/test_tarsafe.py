...
def test_directory_traversal_detection(temp_dir):
    ...
    with pytest.raises(TarSafeException) as exc:
        with TarSafe.open('test.tar') as tar:
            tar.extractall()
    assert "Attempted directory traversal for member: ../../../../../etc/passwd" in str(exc.value)
...
def test_unsafe_link_detection(temp_dir):
    ...
    if os.name != 'nt':  # Skip on Windows
        ...
        with pytest.raises(TarSafeException) as exc:
            with TarSafe.open('test.tar') as tar:
                tar.extractall()
        assert "Attempted directory traversal via link for member: ../outside/sensitive.txt" in str(exc.value)

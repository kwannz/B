...
async def test_base_agent_start_prometheus(base_agent):
    ...
    with patch(
        "tradingbot.backend.trading_agent.agents.base_agent.start_prometheus_server"
    ) as mock_start:
...
def test_base_agent_get_status_no_warnings(base_agent):
    ...
    with patch.multiple(
        "tradingbot.backend.trading_agent.agents.base_agent",
...
def test_base_agent_get_status_with_warnings(base_agent):
    ...
    with patch.multiple(
        ...
        get_inference_latency=Mock(return_value=0.2),
    ):  # Above target 0.1
        status = base_agent.get_status()
...
async def test_base_agent_metrics_tracking(base_agent):
    ...
    with patch.object(base_agent.cache, "get", return_value={"data": "cached"}):
        with patch(
            "tradingbot.backend.trading_agent.agents.base_agent.track_cache_hit"
        ) as mock_hit:
            await base_agent._process_request(request)
    ...
    with patch.object(base_agent.cache, "get", return_value=None):
        with patch(
            "tradingbot.backend.trading_agent.agents.base_agent.track_cache_miss"
        ) as mock_miss:
            await base_agent._process_request(request)
...
async def test_base_agent_concurrent_requests(base_agent):
    ...
    requests = [{"cache_key": f"key_{i}", "data": f"data_{i}"} for i in range(5)]
...
async def test_base_agent_cache_operations(base_agent):
    ...
    for case in test_cases:
        ...
        # Test cache hit
        with patch.object(
            base_agent.cache, "get", return_value=case["data"]
        ) as mock_get:
            result = await base_agent._process_request({"cache_key": case["cache_key"]})
            mock_get.assert_called_once_with(case["cache_key"])
...
async def test_base_agent_large_request(base_agent):
    ...
    large_data = {"cache_key": "large_key", "data": "x" * 1024 * 1024}  # 1MB of data
...
def test_base_agent_get_status_edge_metrics(base_agent):
    ...
    for case in edge_cases:
        with patch.multiple(
            ...
            if case["cache_hit_rate"] <= 0.65:
                assert any(
                    "cache hit rate" in w.lower() for w in status.get("warnings", [])
                )
            if case["error_rate"] >= 0.005:
                assert any(
                    "error rate" in w.lower() for w in status.get("warnings", [])
                )
            if case["inference_latency"] >= 0.1:
                assert any(
                    "inference latency" in w.lower() for w in status.get("warnings", [])
                )
...
async def test_metrics_tracking_concurrent(base_agent):
    ...
    requests = [{"cache_key": f"key_{i}"} for i in range(10)]
...
async def test_prometheus_server_lifecycle():
    ...
    from tradingbot.shared.monitor.prometheus import (
...
async def test_base_agent_request_validation():
    ...
    agent = TestAgent(
        {"name": "test", "type": "base", "enabled": True, "parameters": {}}
    )
    ...
    for req in invalid_requests:
        ...
        assert any(
            msg in str(exc_info.value)
            for msg in ["Request must be a dictionary", "cache_key is required"]
        )

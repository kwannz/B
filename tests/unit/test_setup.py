...
def test_find_packages():
    ...
    assert any('models' in pkg for pkg in packages) or any('shared' in pkg for pkg in packages)
    
    # 验证主要包的存在
    main_packages = ['models', 'shared', 'tests']
    for pkg in main_packages:
        assert any(p.startswith(pkg) or p.endswith(pkg) for p in packages), f"Package {pkg} not found"